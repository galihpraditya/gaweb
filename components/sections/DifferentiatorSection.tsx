  ...
  return (
    <section id="keunggulan" className="py-20 lg:py-24 bg-[#F9F9F9] relative">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Natural photo strip + 4 cards */}
        <div className="grid lg:grid-cols-12 gap-6 lg:gap-8 items-stretch">
          {/* Left Photo Card (5 cols) */}
          <div className="lg:col-span-5 flex flex-col">
            <div className="relative rounded-2xl overflow-hidden border border-slate-200/90 bg-white shadow-xs flex-1 flex flex-col justify-between">
              <div className="aspect-[4/3] lg:aspect-auto lg:flex-1 relative min-h-[260px] overflow-hidden">
                <Image
                  src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=800&auto=format&fit=crop&q=80"
                  alt="Analisis website custom berkinerja tinggi dan cepat"
                  fill
                  className="object-cover photo-natural"
                  sizes="(max-width: 1024px) 100vw, 40vw"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-slate-950/30 via-transparent to-transparent" />
              </div>
              <div className="p-6 bg-white border-t border-slate-100 space-y-1.5">
                <div className="text-base font-bold text-[#092734] flex items-center justify-between">
                  <span>{t.differentiator.cardHeader}</span>
                  <span className="w-2 h-2 rounded-full bg-emerald-500" />
                </div>
                <div className="text-xs sm:text-sm text-slate-600 leading-relaxed">
                  {t.differentiator.cardDesc}
                </div>
              </div>
            </div>
          </div>
